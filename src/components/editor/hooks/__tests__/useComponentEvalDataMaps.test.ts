...
describe('useComponentEvalDataMaps', () => {
  ...
  describe('error', () => {
    ...
    it('returns original error if not cycle', () => {
      const mockComponents: Component[] = [];
      const componentDataDepGraph = generateDepGraph(mockComponents);

      const mockError = new Error('Some error');
      componentDataDepGraph.overallOrder = jest.fn(() => {
        throw mockError;
      });
  ...
  it('evaluates multi-step dependency chains', () => {
    ...
    expect(result.current.componentEvalDataMap).toEqual({
      button1: {
        text: {
          parsedExpression: 'hello21',
          value: 'hello21',
        },
      },
      button2: {
        text: {
          parsedExpression: 'hello2',
          value: 'hello2',
        },
      },
      button3: {
  ...
  it('ignores invalid fields on valid components', () => {
    const mockComponents = [
      {
        name: 'button1',
        type: ComponentType.Button,
  ...
  it('gracefully handles invalid JavaScript within dynamic terms', () => {
    ...
    expect(result.current.componentEvalDataMap).toEqual({
      button1: {
        text: {
          error: new Error('invalidJavascript is not defined'),
        },
      },
    });
    expect(result.current.componentEvalDataValuesMap).toEqual({
